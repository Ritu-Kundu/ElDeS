...
namespace eldes {
  ...
  if (!line.empty()) {
    ...
    while (it != line.end()) {
      ...
      INT first_marker = 0;
      rs = collect_number(line, first_marker, it);
      if (rs != ReturnStatus::SUCCESS) {
	return rs;
      }
      // start of variable region is not odd
      else if (first_marker % 2 != 1) {
	std::cerr << "Invalid marker for beginning of variable/elastic region: " << first_marker << std::endl;
	...
	return rs;
      ...
      INT second_marker = 0;
      rs = collect_number(line, second_marker, it);
      if (rs != ReturnStatus::SUCCESS) {
	return rs;
      }
      // start of variable region is not even
      else if (second_marker % 2 != 0) {
	std::cerr << "Invalid (not odd) marker for separating alleles/strings within variable/elastic region: " << second_marker << std::endl;
	return ReturnStatus::ERR_INVALID_INPUT;
      }
	...
	// Collect the marker 
	marker = 0;
	rs = collect_number(line, marker, it);
	if (rs != ReturnStatus::SUCCESS) {
	  return rs;
	}
      }while (marker == second_marker);

      if (marker != first_marker) {
	std::cerr << "Invalid ending marker of the variable/elastic region" << marker << std::endl;
	return ReturnStatus::ERR_INVALID_INPUT;
      }
  ...
  ReturnStatus Parser::parse_degenerate(const AlphabetType at, const std::string filename, PATTERNS& patterns) {
    std::ifstream infile(filename);
    std::cout << "Reading file: " << filename << std::endl;
    if(!infile.is_open()){
      std::cerr << "Cannot open input file \n";
    ...
    do {
      ...
      if (!line.empty()) {
	if (line[0] != '>') {
	  std::cerr << "Invalid Input: Not a FASTA format: Expected '>' at line number" << lineNum << std::endl;
	  return ReturnStatus::ERR_INVALID_INPUT;
	}
	PATTERN pat;
	pat.first = line.substr(1);
  ...
  ReturnStatus Parser::collect_string (const AlphabetType at, std::string& line, std::string& str, std::string::iterator& it) {
    for (; it != line.end(); ++it ) {
      if (is_valid_elastic_char(at, *it)) {
	str += map_char(toupper(*it));
      }
      else if (isdigit(*it)) {
  ...
  ReturnStatus Parser::collect_number (const std::string& line, INT& num, std::string::iterator& it) {
    std::string str="";
    ...
    try {
      ...
      std::cerr << "Conversion to number failed for: " << str << std::endl;
      return ReturnStatus::ERR_INVALID_INPUT;
    }
